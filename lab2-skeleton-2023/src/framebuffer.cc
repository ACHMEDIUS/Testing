...
void Framebuffer::processEvents(const bool redraw)
{
...
bool Framebuffer::contains(MemAddress addr) const
{
...
void Framebuffer::writeByte(MemAddress addr, uint8_t value)
{
  uint32_t offset = 0;
  FBzone zone = getZone(addr, sizeof(uint8_t), &offset);
  if (zone == FBzone::INVALID)
...
void Framebuffer::writeHalfWord(MemAddress addr, uint16_t value)
{
  uint32_t offset = 0;
  FBzone zone = getZone(addr, sizeof(uint16_t), &offset);
  if (zone == FBzone::INVALID)
...
void Framebuffer::writeWord(MemAddress addr, uint32_t value)
{
  uint32_t offset = 0;
  FBzone zone = getZone(addr, sizeof(uint32_t), &offset);
...
void Framebuffer::writeDoubleWord(MemAddress addr, uint64_t value)
{
  uint32_t offset = 0;
